...
// Modos onde anúncios são autorizados estritamente nas 4 ferramentas solicitadas:
// 1. Criador de Provas ('photo_exam')
// 2. Explicador de Coisas ('explainer')
// 3. Pesquisador ('researcher')
// 4. Tradutor ('translator')
// NÃO há anúncios na tela inicial ('tabs' / 'inicio'), login ou áreas comuns
...
export class AdMobService {
  ...
  public isAdAllowedForMode(currentMode?: string, subStep?: string): boolean {
    ...
    // Valida se corresponde a uma das 4 ferramentas autorizadas (Criador de provas, Explicador de coisas, Pesquisador, Tradutor)
    const isAuthorized =
  ...
  public showAppOpenAd(onDismissed?: () => void): void {
    // Conforme especificação: NÃO colocar anúncios na tela inicial ou áreas comuns.
    // Anúncios são restritos exclusivamente às 4 ferramentas (Criador de provas, Explicador, Pesquisador, Tradutor).
    onDismissed?.();
  }
